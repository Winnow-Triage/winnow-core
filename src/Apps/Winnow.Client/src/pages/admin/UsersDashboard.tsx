...
export default function UsersDashboard() {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesOrg =
      selectedOrg === "all" ||
      user.organizations.some((o) => o.id === selectedOrg);
    const matchesRole =
                ...
                >
                  ...
                  <TableCell>
                    {user.isLockedOut ? (
                      <Badge
                        variant="destructive"
                        className="bg-red-900/30 border-red-500/50 text-red-500"
                      >
                        Locked
                      </Badge>
                    ) : (
                    ...
                    <div className="flex flex-wrap gap-1 justify-center max-w-[150px]">
                      {user.organizations.map((org) => (
                        <Badge
                          key={org.id}
                          variant="secondary"
                          className="text-[10px] px-1 py-0 h-4 bg-muted/50 border-red-900/30"
                        >
                          {org.name}
                        </Badge>
                      ))}
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent
                        align="end"
                        className="border-red-900/50 min-w-[180px]"
                      >
                        <DropdownMenuItem
                          onClick={() => {
      ...
      >
        <AlertDialogContent className="border-red-900/50 bg-background sm:max-w-[500px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground">
              Add or remove <strong>{userToManage?.fullName}</strong> from
              organizations.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold uppercase tracking-wider text-red-400/80">
                Active Memberships
              </h3>
              <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                {userToManage?.organizations.map((org) => (
                  <div
                    key={org.id}
                    className="flex items-center justify-between p-2 rounded-md bg-red-950/20 border border-red-900/30 group"
                  >
                  ...
                  <Select value={orgToJoin} onValueChange={setOrgToJoin}>
                    ...
                    <SelectContent className="border-red-900/50">
                      {organizations
                        .filter(
                          (o) =>
                            !userToManage?.organizations.some(
                              (mo) => mo.id === o.id,
                            ),
                        )
                        .map((org) => (
        ...
        <AlertDialogContent className="border-red-900/50 bg-background sm:max-w-[425px]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground pt-2">
              ...
              <br />
              <br />
              This action is <strong>permanent</strong> and will remove their
              identity record and all organization memberships.
            </AlertDialogDescription>
          </AlertDialogHeader>
