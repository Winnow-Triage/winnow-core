    ...
    <div className="space-y-6">
        ...
        {health?.checks.map((check) => (
          <Card key={check.name} className="border-red-500/20 bg-background/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground text-red-400">
                {check.name}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div className="text-lg font-bold truncate pr-4 text-muted-foreground/80">
                  {check.description &&
                  check.description !== "Healthy" &&
                  check.description !== "Ready"
                    ? check.description
                    : "Operational"}
                </div>
                <Badge
      ...
      <Card className="border-red-500/20 bg-background/50 mt-4">
        <CardHeader>
          ...
          <CardDescription>
            Live diagnostics from the /health/detailed endpoint.
          </CardDescription>
        </CardHeader>
        <CardContent>
