...
export default function AdminSettings() {
  ...
  const handleToggleLock = async () => {
    ...
    try {
      ...
      toast.success(
        `Report successfully ${data.isLocked ? "locked" : "unlocked"}`,
      );
    } catch (error) {
      console.error(error);
    ...
    <div className="space-y-6">
      ...
      <Card className="border-red-500/50 bg-red-500/5 mt-4">
        <CardHeader>
          ...
          <CardDescription className="text-red-400/80">
            These actions can severely impact the system and cannot be undone
            easily. Proceed with extreme caution.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="flex items-center justify-between p-4 border border-red-500/20 rounded-lg bg-background/50">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Permanently delete reports and vector embeddings that belong to
                deleted tenants.
              </p>
            </div>
            <Button
          ...
          <div className="flex items-center justify-between p-4 border border-red-500/20 rounded-lg bg-background/50">
            <div>
              <h3 className="font-semibold text-foreground">
                Force Sync LLM Models
              </h3>
              <p className="text-sm text-muted-foreground">
                Clear cached capability flags and re-query the Ollama/OpenAI API
          ...
          <div className="flex items-center justify-between p-4 border border-red-500/20 rounded-lg bg-background/50">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Block all non-admin traffic and API ingestion. Active users will
      ...
      <Card className="border-blue-500/20 bg-blue-500/5 mt-4">
        <CardHeader>
          ...
          <CardDescription>
            Lookup and manage individual reports bypassing tenant isolation.
          </CardDescription>
        </CardHeader>
              ...
              <div>
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  {report.isLocked ? (
                    <Badge
                      variant="destructive"
                      className="bg-red-600 flex gap-1 items-center"
                    >
                    ...
                    <Badge
                      variant="outline"
                      className="border-green-500/50 text-green-500 bg-green-500/10 flex gap-1 items-center"
                    >
                  ...
                  {report.isOverage && (
                    <Badge
                      variant="destructive"
                      className="bg-amber-600 flex gap-1 items-center"
                    >
                      ⚠️ Overage
                    </Badge>
                  )}
              ...
              <div>
                ...
                >
                  {isToggling ? (
                    ...
                    <>
                      <Unlock className="w-4 h-4 mr-2" /> Unlock Report
                    </>
                  ) : (
                    <>
                      <Lock className="w-4 h-4 mr-2" /> Lock Report
                    </>
                  )}
                </Button>
              </div>
      ...
      <Card className="border-red-500/20 bg-background/50 mt-4">
        <CardHeader>
          ...
          <CardDescription>
            Immutable record of privileged administrative actions.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader className="bg-red-950/20">
                <TableRow className="border-red-900/50 hover:bg-transparent">
                  <TableHead className="text-red-400 w-[180px]">
                    Timestamp
                  </TableHead>
                  <TableHead className="text-red-400 w-[200px]">
                    Actor
                  </TableHead>
                  <TableHead className="text-red-400 w-[150px]">
                    Action
                  </TableHead>
              ...
              <TableBody>
                <TableRow className="border-red-900/20 hover:bg-red-950/10">
                  <TableCell className="text-muted-foreground whitespace-nowrap">
                    2026-02-20 09:42:15 UTC
                  </TableCell>
                  <TableCell className="font-medium">
                    superadmin@example.com
                  </TableCell>
                  <TableCell>
                    <span className="text-orange-400">UpdateTier</span>
                  </TableCell>
                  <TableCell className="text-muted-foreground">
                    Tenant: "Acme Corp" (Free → Pro)
                  </TableCell>
                </TableRow>
                <TableRow className="border-red-900/20 hover:bg-red-950/10">
                  <TableCell className="text-muted-foreground whitespace-nowrap">
                    2026-02-20 09:15:02 UTC
                  </TableCell>
                  <TableCell className="font-medium">
                    superadmin@example.com
                  </TableCell>
                  <TableCell>
                    <span className="text-red-500 font-medium">PurgeData</span>
                  </TableCell>
                  <TableCell className="text-muted-foreground">
                    Tenants: target_id_8f92a, target_id_11b4
                  </TableCell>
                </TableRow>
                <TableRow className="border-red-900/20 hover:bg-red-950/10">
                  <TableCell className="text-muted-foreground whitespace-nowrap">
                    2026-02-19 14:30:00 UTC
                  </TableCell>
                  <TableCell className="font-medium">system</TableCell>
                  <TableCell>
                    <span className="text-blue-400">ForceSync</span>
                  </TableCell>
                  <TableCell className="text-muted-foreground">
                    LLM Models Refresh
                  </TableCell>
