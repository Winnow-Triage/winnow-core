...
export default function OrganizationsDashboard() {
  ...
  const [orgToDelete, setOrgToDelete] = useState<OrganizationSummary | null>(
    null,
  );
  ...
  const handleImpersonate = (orgName: string) => {
    toast.info(`Impersonating tenant: ${orgName}`, {
      description:
        "This feature allows you to securely view the system exactly as this tenant sees it.",
    });
  ...
  const filteredOrganizations = organizations.filter((org) => {
    ...
    const matchesTier =
      tierFilter === "All" || org.subscriptionTier === tierFilter;
          ...
          <TableBody>
            {isLoading ? (
              ...
              filteredOrganizations.map((org) => (
                <TableRow
                  key={org.id}
                  className="border-red-900/20 hover:bg-red-950/10"
                >
                  ...
                  <TableCell>
                    {org.isSuspended ? (
                      <Badge
                        className="bg-orange-500/10 text-orange-500 border-orange-500/20 hover:bg-orange-500/20 text-xs px-2 py-0"
                        variant="outline"
                      >
                        Suspended
                      </Badge>
                      ...
                      <Badge
                        className="bg-green-500/10 text-green-500 border-green-500/20 hover:bg-green-500/20 text-xs px-2 py-0"
                        variant="outline"
                      >
                        Active
                      </Badge>
                  ...
                  <TableCell>
                    <Badge
                      className={getTierBadgeColor(org.subscriptionTier)}
                      variant="outline"
                    >
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent
                        align="end"
                        className="border-red-900/50 min-w-[200px]"
                      >
                        <DropdownMenuItem
                          onClick={() => {
                          ...
                          className="hover:bg-blue-500/20 focus:bg-blue-500/20 cursor-pointer font-medium mb-1"
                        >
                          View Details
                        </DropdownMenuItem>
                        ...
                        <DropdownMenuItem
                          ...
                          className="font-medium text-orange-400 focus:text-orange-400 focus:bg-orange-500/10 mb-2"
                        >
                          Impersonate Tenant
                        </DropdownMenuItem>
                        ...
                        <div className="text-xs text-muted-foreground px-2 py-1.5 font-semibold">
                          Subscription
                        </div>
                        <DropdownMenuItem
                          onClick={() => handleUpdateTier(org.id, "Free")}
                          className="hover:bg-red-500/20 focus:bg-red-500/20 cursor-pointer"
                        >
                          Set to Free
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleUpdateTier(org.id, "Starter")}
                          className="hover:bg-blue-500/20 hover:text-blue-500 focus:bg-blue-500/20 cursor-pointer"
                        >
                          Set to Starter
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleUpdateTier(org.id, "Pro")}
                          className="hover:bg-indigo-500/20 hover:text-indigo-500 focus:bg-indigo-500/20 cursor-pointer"
                        >
                          Set to Pro
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleUpdateTier(org.id, "Enterprise")}
                          className="hover:bg-slate-500/20 hover:text-slate-400 focus:bg-slate-500/20 cursor-pointer"
                        >
                          Set to Enterprise
                        </DropdownMenuItem>
        ...
        <AlertDialogContent className="border-red-900/50 bg-background">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-muted-foreground pt-2">
              This action is completely destructive and cannot be undone. It
              will cascade delete all associated Teams, Projects, Reports,
              Vector Embeddings, and attempt to purge all S3 assets.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          {orgToDelete && (
            <div className="my-4 space-y-3 p-4 bg-red-950/20 border border-red-900/50 rounded-md">
              <p className="text-sm">
                Please type{" "}
                <strong className="text-red-400 font-mono select-all">
                  {orgToDelete.name}
                </strong>{" "}
                to confirm.
              </p>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="border-red-900/50 hover:bg-red-950/20">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
              ...
              disabled={
                !orgToDelete ||
                deleteConfirmationText !== orgToDelete.name ||
                isDeleting
              }
