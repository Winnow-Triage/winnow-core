...
export default function DebugConsole() {
  ...
  const [scenario, setScenario] = useState(
    "A series of login issues where users receive 'Invalid Credentials' but know their password is correct.",
  );
  ...
  const handleGenerateMock = async () => {
    ...
    setMockMessage(null);
    try {
      const { data } = await api.post("/reports/generate-mock", {
        count: mockCount,
        scenario,
      });
  ...
  const handleSimulate = async () => {
    ...
    setMessage(null);
    try {
      const { data } = await api.post("/debug/simulate-traffic", {
        count,
        topic,
      });
    ...
    <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
      <div className="flex flex-col gap-1 text-red-500">
        ...
        <p className="text-muted-foreground flex items-center gap-2">
          <AlertTriangle className="h-4 w-4" /> Tools for developers to test
          system behavior. Not for production use.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Use the LLM to generate realistic, high-quality support reports
            ...
            {currentProject && (
              <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
                ...
                <span>
                  Reports will be generated for project:{" "}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Generate synthetic templates to stress test backend pipeline
              throughput.
            </CardDescription>
            {currentProject && (
              <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
            ...
            <div className="grid w-full max-w-sm items-center gap-1.5">
              ...
              <Select value={topic} onValueChange={setTopic}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Database Timeout">
                    Database Timeout
                  </SelectItem>
