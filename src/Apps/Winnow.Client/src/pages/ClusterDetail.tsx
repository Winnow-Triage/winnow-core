...
export default function ClusterDetail() {
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center p-20 gap-4">
            ...
            <p className="text-muted-foreground animate-pulse">Refining cluster insights...</p>
        </div>
    );
    ...
    const getCriticalityStyles = (score: number | null) => {
        if (!score) return { color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' };
        if (score >= 8) return { color: 'text-red-500', bg: 'bg-red-500/10', border: 'border-red-500/20' };
        if (score >= 5) return { color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20' };
        ...
        <div className="flex flex-col gap-8 max-w-6xl mx-auto w-full pb-20">
                ...
                <div className="relative flex flex-col md:flex-row md:items-end justify-between gap-6 bg-background/60 backdrop-blur-xl border border-white/10 p-8 rounded-2xl shadow-2xl">
                        ...
                        <div className="flex flex-wrap items-center gap-6 mt-2">
                            <div className="flex items-center gap-2 text-muted-foreground bg-white/5 px-3 py-1.5 rounded-full border border-white/5 shadow-inner">
                                ...
                                <span className="text-sm text-foreground">
                                    {cluster.firstSeen ? new Date(cluster.firstSeen).toLocaleDateString() : '—'}
                                    <span className="mx-2 opacity-30">→</span>
                                    {cluster.lastSeen ? new Date(cluster.lastSeen).toLocaleDateString() : '—'}
                                </span>
                            </div>
                            <div className="flex items-center gap-2 text-muted-foreground bg-white/5 px-3 py-1.5 rounded-full border border-white/5 shadow-inner">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[10px] font-black uppercase tracking-widest opacity-50 ml-1">Assigned To</label>
                            <div className="flex items-center gap-2">
                                <input
                                    type="text"
                                    ...
                                    onBlur={async (e) => {
                                        if (e.target.value !== cluster.assignedTo) {
                                            await api.post(`/clusters/${cluster.id}/assign`, { assignedTo: e.target.value });
                        ...
                        <CardHeader className="flex flex-row items-start justify-between p-8 pb-4 relative z-10">
                            <div className="flex flex-col gap-3">
                                ...
                                {cluster.criticalityScore && (
                                    <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border ${criticality.border} ${criticality.bg} shadow-inner`}>
                                        <div className={`h-2 w-2 rounded-full animate-pulse ${criticality.color.replace('text', 'bg')}`} />
                                        <span className={`text-sm font-bold tracking-tight ${criticality.color}`}>
                            ...
                            <div className="flex items-center gap-2">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="rounded-xl border-white/10 backdrop-blur-xl bg-background/90 shadow-2xl">
                                        <DropdownMenuItem disabled={isGeneratingSummary} onClick={handleGenerateSummary} className="p-3">
                                            <RotateCw className={`mr-2 h-4 w-4 ${isGeneratingSummary ? 'animate-spin' : ''}`} />
                                            Regenerate Analysis
                                        </DropdownMenuItem>
                                        <DropdownMenuSeparator className="bg-white/10" />
                                        <DropdownMenuItem className="text-red-600 p-3" disabled={isClearingSummary} onClick={handleClearSummary}>
                            ...
                            {cluster.summary ? (
                                    ...
                                    <div className="prose prose-lg dark:prose-invert max-w-none leading-relaxed text-foreground/80 font-medium">
                                        <ReactMarkdown
                                            components={{
                                                ...
                                                strong: ({ node, ...props }) => <strong className="text-foreground font-black bg-purple-500/5 px-1 rounded" {...props} />,
                                                ul: ({ node, ...props }) => <ul className="space-y-2 list-disc list-inside marker:text-purple-500" {...props} />,
                                            }}
                                        >
                                            {cluster.summary}
                                        </ReactMarkdown>
                                    ...
                                    {cluster.criticalityReasoning && (
                                        <Card className="bg-white/5 dark:bg-white/[0.03] border-white/5 dark:border-white/10 border-l-4 border-l-purple-500 p-6 rounded-2xl shadow-inner">
                                            <p className="text-sm italic leading-relaxed text-muted-foreground font-medium">
                                                <span className="text-2xl text-purple-500 font-serif leading-none mr-1">“</span>
                                                {cluster.criticalityReasoning}
                                                <span className="text-2xl text-purple-500 font-serif leading-none ml-1">”</span>
                                            </p>
                                        </Card>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-xl font-bold tracking-tight text-muted-foreground">No insight generated yet.</p>
                                        <Button variant="outline" className="rounded-full shadow-sm" onClick={handleGenerateSummary} disabled={isGeneratingSummary}>
                    ...
                    <Card className="border border-white/5 dark:border-white/10 shadow-2xl bg-white/5 dark:bg-white/[0.02] backdrop-blur-sm rounded-3xl overflow-hidden">
                        <CardHeader className="p-6 pb-2">
                            <CardTitle className="text-xl font-bold flex items-center gap-3 tracking-tight">
                                ...
                                Impacted Reports
                                <Badge variant="secondary" className="ml-auto rounded-full px-3">{cluster.reports.length}</Badge>
                            </CardTitle>
                        </CardHeader>
                                    ...
                                    <Link key={report.id} to={`/reports/${report.id}`} className="group shrink-0 p-4 bg-white/5 border border-white/5 hover:border-blue-500/30 hover:bg-white/10 rounded-2xl transition-all duration-300 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col gap-2 relative z-10">
                                            <div className="flex items-start justify-between gap-2">
                                                ...
                                                <Badge variant="outline" className="text-[9px] h-fit px-1.5 py-0 bg-white/5 border-white/10 opacity-60">
                                                    {report.status}
                                                </Badge>
                                            </div>
            ...
            <AlertDialog open={confirmAction.isOpen} onOpenChange={(open) => setConfirmAction(p => ({ ...p, isOpen: open }))}>
                <AlertDialogContent className="rounded-2xl border-white/10 backdrop-blur-2xl bg-background/80 shadow-2xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-2xl font-black tracking-tight">{confirmAction.title}</AlertDialogTitle>
...
function ClusterExportMenu({ clusterId, projectId, onExport }: { clusterId: string, projectId: string, onExport: () => void }) {
    ...
    const exportMutation = useMutation({
        mutationFn: async (configId: string) => {
            const { data } = await api.post(`/clusters/${clusterId}/export`, { configId });
            return data;
        },
        onSuccess: (data: any) => {
        ...
        onError: (error: any) => {
            const displayMsg = error.response?.data?.error || error.message || "Unknown error";
