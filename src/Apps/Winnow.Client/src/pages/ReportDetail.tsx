...
export default function ReportDetail() {
  ...
  if (error || !report)
    return <div className="p-8 text-red-500">Error loading report.</div>;
                      ...
                      {(() => {
                        try {
                          ...
                          return Object.entries(metadata)
                            ...
                            ) // Filter logs and generic context if redundant
                            .map(([key, value]) => (
                              <React.Fragment key={key}>
                                ...
                                <span
                                  className="text-xs text-right font-mono truncate"
                                  title={String(value)}
                                >
                                  {String(value)}
                                </span>
              ...
              <CardHeader className="bg-muted/30">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  ...
                  <Badge variant="neutral" className="ml-auto text-xs h-5">
                    {report.assets.length} file
                    {report.assets.length > 1 ? "s" : ""}
                  </Badge>
                </CardTitle>
