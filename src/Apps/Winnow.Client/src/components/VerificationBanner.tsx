...
export default function VerificationBanner() {
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            const message = error.response?.data?.errors?.[0]?.description || error.response?.data?.message || "Failed to resend verification email.";
            toast.error(message);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <div className="bg-amber-50 border-b border-amber-200 text-amber-800 px-4 py-2 flex items-center justify-center gap-4 text-sm font-medium">
            <div className="flex items-center gap-2">
                ...
                <span>Please verify your email address to unlock team collaboration and issue exports.</span>
            </div>
            <Button
                variant="outline"
                size="sm"
