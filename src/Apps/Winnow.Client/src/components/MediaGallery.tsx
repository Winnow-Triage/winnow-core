...
import { Play, X, ZoomIn, Download, FileText, Film, Image } from "lucide-react";
                    ...
                    {attachments.length > 1 && (
                        <>
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
                                    setLightboxIndex((lightboxIndex - 1 + attachments.length) % attachments.length);
                                    setZoomed(false);
                                }}
                    ...
                    >
                        {isImage(current.type) ? (
                            <img
                                ...
                                className={`max-h-[85vh] max-w-[90vw] object-contain rounded-lg shadow-2xl cursor-zoom-in transition-transform duration-300 ${zoomed ? "scale-150 cursor-zoom-out" : ""
                                    }`}
                            />
                            ...
                            <div className="bg-card rounded-lg p-8 max-w-md text-center shadow-2xl">
                                ...
                                <p className="text-foreground font-medium mb-2">{current.filename}</p>
                                <a
                                    href={current.url}
                                    download={current.filename}
